  ...
  return (
    <div className="min-h-screen px-4 py-16 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl space-y-10">
        <FadeIn delay={0.2}>
          <div className="matchday-hero rounded-3xl p-8 sm:p-10">
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-3 text-xs text-gray-200">
                <span className="rounded-full border border-white/10 bg-white/10 px-3 py-1">Sondages live</span>
                <span className="rounded-full border border-white/10 bg-white/10 px-3 py-1">Pronostics</span>
                <span className="rounded-full border border-white/10 bg-white/10 px-3 py-1">Défis fans</span>
              </div>
            </div>
          </div>
        </FadeIn>
          ...
          <ScaleIn delay={0.3}>
            <div className="glass rounded-2xl p-6 sm:p-8 border border-white/10">
              <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                ...
                <span className="rounded-full bg-red-500/20 px-3 py-1 text-xs text-red-200">Modération active</span>
              </div>
              <FanWallComposer />
              <div className="space-y-4">
                {wallPosts.map((post, index) => (
                  <ScaleIn key={post.id} delay={0.35 + index * 0.05}>
                    <div className="rounded-xl bg-white/5 p-4 border border-white/10 hover:border-white/20 transition-colors">
            ...
            <ScaleIn delay={0.4}>
              <div className="glass rounded-2xl p-6 border border-white/10">
                ...
                <div className="space-y-3 text-sm text-gray-300">
                  {chants.map((chant) => (
                    <div key={chant.id} className="flex items-center justify-between rounded-lg bg-white/5 px-4 py-3 border border-white/10">
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {highlights.map((highlight, index) => (
                  <ScaleIn key={highlight.id} delay={0.4 + index * 0.05}>
                    <div className="rounded-xl bg-white/5 p-4 border border-white/10 hover:border-white/25 transition-colors h-full flex flex-col">
                      ...
                      <p className="mt-2 text-sm text-gray-300 flex-1">{highlight.detail}</p>
                      <button
                        className="mt-4 inline-flex items-center justify-center rounded-lg bg-white/10 px-3 py-2 text-xs font-semibold text-white hover:bg-white/20 transition-colors"
                        type="button"
                      >
                        Lancer
                      </button>
